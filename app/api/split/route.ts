...
반드시 '오직 JSON만': {"reply":"무엇을 어떻게 바꿨는지 1~2문장","items":[{"id":"입력id","name":"...","durationMin":30,"deadline":"2026-01-10"}]}`;
    ...
    const rc = (body as { replan?: { date?: string; availableMin?: number; plannedMin?: number; overMin?: number; tasks?: { id: string; name: string; durationMin?: number; deadline?: string; priority?: number; schedulingType?: string; dependsOn?: string[]; projectDeadline?: string }[]; upcoming?: { date: string; freeMin: number }[] } }).replan ?? {};
...
반드시 '오직 JSON만': {"reply":"무엇을 왜 옮겼는지 1~2문장(한국어)","moves":[{"id":"taskId","toDate":"YYYY-MM-DD"}]}`;
    user = `오늘: ${rc.date ?? today}\n가용(분): ${rc.availableMin ?? 0} / 계획(분): ${rc.plannedMin ?? 0} / 초과(분): ${rc.overMin ?? 0}\n\n오늘 배치된 옮길 수 있는 Task (id | 이름 | 소요분 | 기한 | 우선순위 | 선행task | 프로젝트마감):\n${(rc.tasks ?? []).map(t => `- ${t.id} | ${t.name} | ${t.durationMin ?? '?'} | ${t.deadline ?? '없음'} | ${t.priority ?? 0} | ${(t.dependsOn ?? []).join(',') || '없음'} | ${t.projectDeadline ?? '없음'}`).join('\n') || '(없음)'}\n\n앞으로 14일 날짜별 남은 여유(분):\n${(rc.upcoming ?? []).map(u => `- ${u.date}: ${u.freeMin}`).join('\n')}\n\n제약: 선행task가 있으면 그 이후 날짜로만, 가능하면 프로젝트마감 이내로 배치. 초과분을 해소하도록 옮길 Task와 목적지 날짜를 제안해줘.`;
