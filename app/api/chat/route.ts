  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        // 2차 패스는 '계획을 반영하려는 요청'일 때만 → 단순 질문·조언엔 로딩/버튼 안 뜨게.
        // 판단: (1) 사용자 메시지가 추가/생성/미루기 등 '행동 요청'이거나, (2) AI 응답에 제안 문구·날짜가 있으면 계획으로 간주.
        const lastUser = [...(messages as { role: string; content: string }[])].reverse().find(m => m.role === 'user')?.content ?? '';
        const userWantsPlan = /추가|생성|만들|짜|미루|연기|옮겨|옮기|배치|반영|정리|종료|넣어|잡아|세워|추진|진행|계획|일정|업무|프로젝트|task/i.test(lastUser);
        const looksLikePlan = /반영할까요|반영하면|추가할까요|추가하면|미룰까요|만들까요|정리할까요|버튼을 누르|이렇게 (반영|추가|정리|배치|진행)|\d{4}-\d{2}-\d{2}/.test(full);
        if (routineMode && (userWantsPlan || looksLikePlan) && !/%%%[A-Z_]+%%%/.test(full)) {
