...
export interface AppData {
  ...
  timerTimes?: Record<string, Record<string, number>>; // 날짜별("YYYY-MM-DD") → taskId → 누적 초. 기기 간 동기화(합산은 max-merge)
  timerFocus?: Record<string, number>; // 날짜별 하루 총 집중(타이머) 초 — getDayTotalSeconds용. 기기 간 동기화(max-merge)
  planReview?: { lastAt?: string; intervalDays?: number }; // 정기 계획 검토 알림: 마지막 검토일 + 알림 주기(일). 기기 간 동기화
