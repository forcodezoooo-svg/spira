...
- ★ 후속 확답에도 마커 필수 ★: 앞서 미리보기를 보여준 뒤 사용자가 "응", "그렇게 해줘", "반영해줘", "좋아", "ㅇㅋ" 같은 **짧은 확답**을 하면, 설명을 다시 길게 쓸 필요 없이 **한 문장 요약 + 반드시 %%%QUARTER_PLAN%%% 마커 + JSON**을 다시 출력하세요. 확답에 마커를 빠뜨리면 사용자가 반영을 못 합니다. 조금이라도 '계획을 실제로 반영'하는 뉘앙스면 마커를 붙이는 쪽을 택하세요.
