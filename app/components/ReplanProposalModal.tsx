...
export default function ReplanProposalModal({
  proposal, onApply, onClose, onRequestAI, aiBusy, aiReply, aiActive, aiEnabled,
}: {
  proposal: ReplanProposal;
  onApply: (proposal: ReplanProposal) => void;
  onClose: () => void;
  onRequestAI?: () => void;   // AI에게 더 나은 제안 요청
  aiBusy?: boolean;
  aiReply?: string;           // AI 설명
  aiActive?: boolean;         // 현재 표시 중인 제안이 AI가 만든 것인지
  aiEnabled?: boolean;
}) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(22,33,30,0.4)' }} onClick={onClose}>
      <div className="bg-white rounded-[24px] w-full max-w-[560px] max-h-[90vh] overflow-y-auto p-6" style={{ boxShadow: 'var(--spira-shadow-lg)' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2 mb-3">
          <span className="text-[11px] font-bold rounded-full px-2.5 py-0.5" style={aiActive ? { backgroundColor: '#F3F0FF', color: '#7C3AED' } : { backgroundColor: '#F0F0EA', color: '#5B6560' }}>{aiActive ? 'AI 제안' : '기본 제안'}</span>
          {aiEnabled && onRequestAI && (
            <button onClick={onRequestAI} disabled={aiBusy} className="flex items-center gap-1 text-[12px] font-bold px-2.5 py-1 rounded-full transition-colors disabled:opacity-50" style={{ backgroundColor: '#F3F0FF', color: '#7C3AED' }}>
              {aiBusy ? <span className="w-3 h-3 rounded-full border-2 border-current border-t-transparent animate-spin" /> : <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3l1.73 5.27L19 10l-5.27 1.73L12 17l-1.73-5.27L5 10l5.27-1.73L12 3z" /></svg>}
              AI 제안 받기
            </button>
          )}
        </div>
        {aiActive && aiReply && (
          <p className="text-[12px] mb-3 rounded-xl px-3 py-2 leading-relaxed" style={{ backgroundColor: '#F7F5FF', color: '#5B4B8A' }}>{aiReply}</p>
        )}
