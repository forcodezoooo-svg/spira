...
const GoalsRoadmap = forwardRef<GoalsRoadmapHandle, Props>(function GoalsRoadmap(
  ...
  const saveSelectedAsGroup = () => {
    ...
    setGroupSaveOpen(false); setGroupSelMode(false); setGroupSelIds(new Set()); setGroupSaveName('');
  };
    ...
    return (
      <div key={s.id} data-teach="kb-task" onClick={e => e.stopPropagation()} draggable onDragStart={() => setKbDrag(s.id)} onDragEnd={() => { setKbDrag(null); setKbDragOver(null); }}
        ...
        className="group bg-white border rounded-lg p-2.5 cursor-grab active:cursor-grabbing" style={highlightIds?.has(s.id)
          ...
          : (!s.deadline && !s.date)
          ? { borderColor: '#DADAD3', backgroundColor: '#EFEFEB', boxShadow: '0 1px 2px rgba(0,0,0,0.04)', opacity: kbDrag === s.id ? 0.4 : 1 }
  ...
  return (
    <div className={`flex flex-col ${cardClassName}`}>
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        <div className="flex gap-3 h-full transition-transform duration-300 ease-out" style={{ transform: kanban ? 'translateX(calc(116px - 100%))' : 'translateX(0px)' }}>
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <button onClick={() => setOffOpen(o => !o)} className="text-[12px] font-semibold rounded-full px-2.5 py-1.5 transition-colors flex-shrink-0" style={offOpen ? { backgroundColor: '#FBE7C6', color: '#96631A' } : { backgroundColor: '#F0F0EA', color: '#5B6560' }} title="오프 기간(휴가 등) 설정">off day 설정</button>
          </div>
          {offOpen && (
      ...
      <div ref={scrollRef} onScroll={e => updateVisLabel(e.currentTarget)} className="flex-1 min-h-0 overflow-auto overscroll-contain border rounded-xl" style={{ borderColor: 'var(--spira-border-subtle)' }} onDragOver={e => { if (dragPayloadRef.current) e.preventDefault(); }} onDrop={onTrackDrop}>
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          {!barScope && <div className="flex items-center gap-2 mb-2 flex-shrink-0 flex-wrap">
            ...
            {kbColsView.length > 0 && (groupSelMode ? (
              <>
                <span className="text-[11px] font-bold flex-shrink-0" style={{ color: '#7C3AED' }}>저장할 카테고리를 클릭하세요 · {groupSelIds.size}개</span>
                <button onClick={() => { if (groupSelIds.size) { setGroupSaveName(''); setGroupSaveOpen(true); } }} disabled={groupSelIds.size === 0} className="flex items-center gap-1 rounded-full px-3 py-1.5 text-[12px] font-bold flex-shrink-0 transition-transform hover:-translate-y-0.5 disabled:opacity-40" style={{ backgroundColor: '#9DFE3B', color: '#16211E' }} title="선택한 카테고리를 그룹 템플릿으로 저장">저장{groupSelIds.size > 0 ? ` (${groupSelIds.size})` : ''}</button>
                <button onClick={() => { setGroupSelMode(false); setGroupSelIds(new Set()); }} className="rounded-full px-3 py-1.5 text-[12px] font-bold flex-shrink-0 transition-colors" style={{ backgroundColor: '#F0F0EA', color: '#5B6560' }}>취소</button>
              </>
            ) : (
              <button onClick={() => { setKbFlat(false); setGroupSelMode(true); setGroupSelIds(new Set()); }} data-teach="kb-template" className="flex items-center gap-1 rounded-full px-3 py-1.5 text-[12px] font-bold flex-shrink-0 transition-colors" style={{ backgroundColor: '#F3F0FF', color: '#7C3AED' }} title="보드에서 저장할 카테고리를 직접 골라 템플릿으로 저장">
          ...
          <div ref={boardRef} onClick={e => { if (barScope && e.target === e.currentTarget) { if (kbScope.todoId) scrollToTodoRef.current = kbScope.todoId; setKanban(true); } }} className="flex-1 min-h-0 flex gap-3 overflow-x-auto pb-1">
            ...
            <div key={col.todoId} data-col-todo={col.todoId} data-ask data-ask-label={`${col.p.wsName ? col.p.wsName + ' · ' : ''}카테고리 · ${col.area}`} data-ask-content={`[비즈니스: ${col.p.wsName || '내 비즈니스'}] 카테고리 '${col.area}'${col.goalSub ? `: ${col.goalSub}` : ''}`} className="relative flex flex-col min-h-0 w-[317px] flex-shrink-0 rounded-xl border-2 transition-shadow" style={(groupSelMode && groupSelIds.has(col.todoId))
              ...
              {groupSelMode && (() => { const gSel = groupSelIds.has(col.todoId); return (
                <div onClick={e => { e.stopPropagation(); setGroupSelIds(prev => { const n = new Set(prev); if (n.has(col.todoId)) n.delete(col.todoId); else n.add(col.todoId); return n; }); }} className="absolute inset-0 z-30 cursor-pointer rounded-xl" title="클릭해서 저장 선택/해제" style={{ backgroundColor: gSel ? 'rgba(124,58,237,0.06)' : 'transparent' }}>
                  <span className="absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center" style={{ backgroundColor: gSel ? '#7C3AED' : 'rgba(255,255,255,0.92)', border: `2px solid ${gSel ? '#7C3AED' : '#C7CEC7'}` }}>
                    {gSel && <svg className="w-3.5 h-3.5" viewBox="0 0 12 12" fill="none"><path d="M2 6l2.5 2.5L10 3" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                  </span>
                </div>
              ); })()}
              {/* 헤더: 업무영역(큰) + 산출물(작은) + 기한 */}
      ...
      {/* 그룹 저장: 이름 설정 팝업 (카테고리는 보드에서 이미 선택했음) */}
      {groupSaveOpen && (
