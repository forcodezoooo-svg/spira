                        ...
                        return (
                          <div key={s.id} draggable onDragStart={() => setKbDrag(s.id)} onDragEnd={() => setKbDrag(null)}
                            ...
                            <div className="flex items-start gap-1.5">
                              ...
                              <button onClick={() => kbEditTask(col, s)} title="task 수정 (이름·소요시간·성격·우선순위)" className="font-semibold flex-1 min-w-0 break-words text-left" style={{ fontSize: 14.3, color: showDone ? '#9AA39D' : '#16211E', textDecoration: showDone ? 'line-through' : 'none' }}>{s.name}</button>
                              ...
                              <div className="mt-1.5 ml-5 space-y-0.5">
                                {units.map(u => (
                                  <div key={u.id} className="group/u flex items-center gap-1.5">
                                    ...
                                    <button onClick={() => kbEditUnit(col, s, u)} title="수정" className="truncate flex-1 min-w-0 text-left" style={{ fontSize: 12.1, color: u.done ? '#9AA39D' : '#5B6560', textDecoration: u.done ? 'line-through' : 'none' }}>{u.name}</button>
