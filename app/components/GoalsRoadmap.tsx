    ...
    <div className={`flex flex-col ${cardClassName}`}>
      ...
      <div ref={scrollRef} onScroll={e => updateVisLabel(e.currentTarget)} className="flex-1 min-h-0 overflow-auto overscroll-contain border rounded-xl" style={{ borderColor: 'var(--spira-border-subtle)' }} onDragOver={e => { if (dragPayloadRef.current) e.preventDefault(); }} onDrop={onTrackDrop}>
        <div className="relative" style={{ width: LABEL_W + contentWidth }}>
          ...
          <div onClick={e => { if ((e.target as HTMLElement).closest('button, input, select, textarea, a')) return; setKanban(!kanban ? true : false); }} className="relative flex-shrink-0 flex flex-col min-w-0 rounded-[22px] border bg-white p-4" style={{ width: 'calc(100% - 64px)', borderColor: 'var(--spira-border-subtle)', boxShadow: 'var(--spira-shadow-lg)' }}>
